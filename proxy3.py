...
async def start(event):
    ...
    try:
        ...
        welcome_message = (
            "👋 سلام!\n\n"
            "به ربات Proxy Collector خوش آمدید. این ربات به شما کمک می‌کند تا به سادگی و سرعت به لیست پروکسی‌های مختلف دسترسی پیدا کنید.\n\n"
            "🔍 برای مشاهده لیست پروکسی‌های کشورهای مختلف، دستور /list را ارسال کنید.\n"
            "📜 همچنین می‌توانید از دستورات زیر استفاده کنید تا پروکسی‌های کشورها یا لایه‌های مختلف را دریافت کنید.\n"
            "💡 برای مشاهده دستورهای بیشتر، از دستور /list استفاده کنید."
        )
...
async def send_proxy_list(event):
    ...
    try:
        ...
        proxy_list = (
            ...
            "\U0001F30F آی‌پی‌وی۴: /ipv4\n"
            "\U0001F30F آی‌پی‌وی۶: /ipv6\n"
        )
        await event.reply(proxy_list)

    except:
        # اگر کاربر عضو کانال نبود
        await event.reply('🚨 برای استفاده از این دستور، ابتدا باید در کانال @External_Net عضو شوید.')
...
@client.on(events.NewMessage(pattern='/ipv4'))
async def send_ipv4_proxies(event):
    await send_proxies(event, 'ipv4')

@client.on(events.NewMessage(pattern='/ipv6'))
async def send_ipv6_proxies(event):
    await send_proxies(event, 'ipv6')
